...
if __name__ == '__main__':
    ...
    #         model = tf.keras.models.load_model(f'../results/fine/random/en_freeze/random/unet_140_fine_{i}_{j}',
    #                                            custom_objects={'dice_loss': dice_loss,
    #                                                            'iou': iou,
    #                                                            'tree_iou': tree_iou})
    #         acc1, acc2 = [], []
    ...
    #         with pd.ExcelWriter(r'../results/fine/random/en_freeze/random/en_freeze_random.xlsx', mode='a',
    ...
    # with pd.ExcelWriter(r'../results/fine/random/en_freeze/random/en_freeze_random.xlsx', mode='a',
    #                     engine='openpyxl', if_sheet_exists='replace') as writer:
    #     df_o.to_excel(writer, sheet_name=f'fine_random_overall')
    model = tf.keras.models.load_model(f'../results/high_low_2/unet_res_high_2',
                                       custom_objects={'dice_loss': dice_loss,
                                                       'iou': iou,
                                                       'tree_iou': tree_iou})
    acc1, acc2 = [], []
    ...
    print(df)
    print(np.mean(acc1), np.mean(acc2))
    with pd.ExcelWriter(r'../results/high_low_2/r1.xlsx', mode='a',
                        engine='openpyxl', if_sheet_exists='replace') as writer:
        df.to_excel(writer, sheet_name=f'high_retrain_test')
    del model

...
if __name__ == '__main__':
    ...
    with strategy.scope():
        ...
        model = U_Net(input_shape=(width, width, 7), n_classes=2, recurrent=False, residual=True, attention=True)
        model.compile(optimizer=optimizer, loss=[loss_fn], metrics=[iou])
    model.summary()

    learning_rate_scheduler = tf.keras.callbacks.LearningRateScheduler(lr_cosine_decay, verbose=0)
    # tensorboard
    tensorboard_callbacks = tf.keras.callbacks.TensorBoard(log_dir='tb_callback_dir/unet_res_att_softmax_low',
                                                           histogram_freq=1)
    ...
    # model.save('model.h5')
    model.save_weights('checkpoints/ckpt-unet_res_att_softmax_low')

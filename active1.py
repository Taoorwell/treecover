...
def initial_model_train(initial_dataset, validation_dataset):
    if os.path.exists(r'checkpoints/active/high/delta_decay/unet_active_1'):
        model = tf.keras.models.load_model(r'checkpoints/active/high/delta_decay/unet_active_1',
        ...
        model.save(r'checkpoints/active/high/delta_decay/unet_active_1')
    return model
...
def model_test(model, images, masks, images_ids, inf, n):
    ...
    with pd.ExcelWriter(r'checkpoints/active/high/delta_decay/r.xlsx',
...
if __name__ == '__main__':
    ...
    for i in np.arange(2, 8):
        ...
        images, masks, prob, df, image_id_selected = model_pred(model,
                                                                ...
                                                                inf=5,
                                                                delta=delta)
        with pd.ExcelWriter(r'checkpoints/active/high/delta_decay/r.xlsx', engine='openpyxl', mode='a',
        ...
        if os.path.exists(f'checkpoints/active/high/delta_decay/unet_active_{i}'):
            model = tf.keras.models.load_model(f'checkpoints/active/high/delta_decay/unet_active_{i}',
                                               custom_objects={'dice_loss': dice_loss,
                                                               'iou': iou,
                                                               'tree_iou': tree_iou},
                                               compile=True)
        else:
            model = tf.keras.models.load_model(f'checkpoints/active/high/delta_decay/unet_active_{i-1}',
                                               custom_objects={'dice_loss': dice_loss,
                                                               'iou': iou,
                                                               'tree_iou': tree_iou},
                                               compile=True)
            ...
            model.save(f'checkpoints/active/high/delta_decay/unet_active_{i}')
